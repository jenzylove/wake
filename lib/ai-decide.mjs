...
export function enforce(ai, candidates, { authorised = false } = {}) {
  ...
  if (!c) refusals.push(`candidate ${ai.candidate} is not one code measured`)
    ...
    const residual = c.modeledDeltaPct == null ? null : c.modeledDeltaPct - (c.marketDeltaPct ?? 0)
    if (residual === null || residual < POLICY_THRESHOLDS.minResidualPct) {
      refusals.push(`edge after what the market already did is ${residual === null ? "unknown" : `${residual.toFixed(2)}%`}, below ${POLICY_THRESHOLDS.minResidualPct}%`)
    }
