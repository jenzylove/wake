...
- TRADE_DIRECT: trade the instrument the event lands on. For an exploit this is always a short of the drained protocol. For a flow event (SUPPLY_TO_EXCHANGE) you also choose the side, and the side is the judgement: a holder distributing is sell pressure, while a market maker or exchange refilling inventory is not, and often precedes the opposite. If you cannot tell which, do not trade.
...
Return JSON only:
{"action": one of ${JSON.stringify(ACTIONS)}, "candidate": integer index or null, "side": "LONG" or "SHORT" or null (only the sides the candidate lists), "confidence": integer 0-100, "thesis": string (<=400 chars), "invalidation": string (<=240 chars), "rationale": string (<=500 chars)}`
...
export function validateDecision(r, model = null) {
  ...
  const side = r.side === "LONG" || r.side === "SHORT" ? r.side : null
  return {
    model, action, candidate, side, confidence,
...
export function enforce(ai, candidates, { authorised = false } = {}) {
  ...
  if (!c) refusals.push(`candidate ${ai.candidate} is not one code measured`)
    ...
    // The side is the model's to choose where the candidate offers a choice; code only checks it is one on offer.
    const sides = Array.isArray(c.sides) && c.sides.length ? c.sides : ["SHORT"]
    if (!ai.side) refusals.push("no side was chosen")
    else if (!sides.includes(ai.side)) refusals.push(`side ${ai.side} is not one this candidate offers`)
  ...
  if (refusals.length) return { decision: "NO_TRADE", chosen: null, side: null, refusals, proposed }
  return { decision: proposed, chosen: c, side: ai.side, refusals: [], proposed }
}

/**
 * If code would refuse every possible trade whatever the model said, return why; otherwise null.
 * The model is only consulted when its answer could change the outcome. This saves the call and
 * never changes a decision: the result is the same NO_TRADE enforce() would return.
 */
export function codeRefusesAnyTrade(candidates) {
  if (!candidates.length) return "no listed instrument to trade, so there is nothing to decide"
  const why = candidates.map((c) => {
    if (c.demoListed === false) return `${c.instrument} is not listed on Bitget Demo`
    if (!c.sizing?.computable) return "no position size can be computed"
    if (c.sameSenderOpen) return "this depositor already has an open position"
    const minEdge = Number.isFinite(c.minEdgePct) ? c.minEdgePct : POLICY_THRESHOLDS.minResidualPct
    const edge = c.modeledDeltaPct == null ? null : c.modeledDeltaPct - Math.max(0, c.marketDeltaPct ?? 0)
    if (edge === null || edge < minEdge) return `edge ${edge === null ? "unknown" : `${edge.toFixed(2)}%`} is below the ${minEdge}% bar`
    return null
  })
  return why.every(Boolean) ? why.join("; ") : null
}
